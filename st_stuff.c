...
#include "r_draw.h"   //vrodic:11-01-98:sets up borderpatchname in ST_Init()
...
//vrodic:18-03-98:quickie routine to fill the space around the statusbar when
...
void ST_drawStatusBarBorders(void)
{
    ...
    i=SCREENHEIGHT-ST_HEIGHT;   //match with the backtile around the viewwindow
	...
	if (SCREENWIDTH&63)
	{
	    memcpy (dest, src+((i&63)<<6), SCREENWIDTH&63);
	    dest += (SCREENWIDTH&63);
	}
	i++;
    }
...
void ST_refreshBackground(void)
{
    ...
    if (st_statusbaron)
    {
	//vrodic:18-03-98:fill the gaps around the statusbar
	...
	//vrodic:18-03-98:draw space around status bar too
...
//  WriteDebug("ST_Drawer - ST_doRefresh...\n");
    ...
    // if (st_firsttime) ST_doRefresh(); // vrodic : new automap
...
void ST_Init (void)
{
    ...
    //vrodic:18-03-98:sets the default borderpatch.
    if ( gamemode == commercial)
	 strcpy(borderpatchname,"RROCK17");    // DOOM II border patch.
    //	  strcpy(borderpatchname,"GRNROCK");	// DOOM II border patch.
    else
	strcpy(borderpatchname,"FLOOR7_2");	// DOOM border patch.
}
