...
extern int			crosshair; // vrodic : crosshair
extern int			newhud;    // vrodic : new automap
extern int			rotatemap; // vrodic : automap rotation
...
char *cmdlist[] = { "iddqd : god mode",
                    ...
                    "crosshair : toggle crosshair",// vrodic	
					"newmap : toggle new map style", // vrodic	
					"maprotate : toggle map rotation",// vrodic
...
int CO_HandleCommand(char *cmd)
   {
	...
	// vrodic : crosshair
	if (stricmp(cmd, "crosshair") == 0)
	{
        static char buf[ST_MSGWIDTH];
		crosshair = !crosshair;
		sprintf(buf, "CROSSHAIR %s", onoff[crosshair]);
        plyr->message = buf;
		return FALSE;

	}
	// and automap
	if (stricmp(cmd, "newmap") == 0 )
	{
        static char buf[ST_MSGWIDTH];
		newhud = !newhud;
		sprintf(buf, "NEW AUTOMAP %s", onoff[newhud]);
        plyr->message = buf;
		return FALSE;
	}
	if (stricmp(cmd, "maprotate") == 0 )
	{
        static char buf[ST_MSGWIDTH];
		rotatemap = !rotatemap;
		sprintf(buf, "AUTOMAP ROTATION %s", onoff[rotatemap]);
        plyr->message = buf;
		return FALSE;
	}
