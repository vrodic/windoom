...
// vrodic : enhanced automap stuff
//int newhud=false;
int newhud=true;

boolean newhupd=false;

//extern int crosshair; checked in hu_stuff.c now
...
void AM_Stop (void)
{
	...
	// vrodic: restore crosshair // now checked in hu_stuff.c
...
void AM_Start (void)
{
    ...
    // vrodic: kill crosshair in automap // now checked in hu_stuff.c
	//crosshairstate = crosshair;
	/*if (!newhud)*/ //crosshair = false; // may or may not?
	///////////////////////////////////
    if (!stopped) AM_Stop();
    stopped = false;
...
void AM_drawMline( mline_t *ml,int color )
   ...
   // vrodic : enh 
   if ( automapactive == 1 )
      color = GREENS + (GREENRANGE/2)+(color%(GREENRANGE/2));
   ////
...
void AM_drawWalls(void)
{
	...
	l.b.x = lines[i].v2->x;
	l.b.y = lines[i].v2->y;
	// vrodic : rotate map code
    if (rotatemap)
    {
      // rotate map point around player location
      // negative angles rotate 2D points counter clockwise
      l.a.x -= plr->mo->x;
      l.b.x -= plr->mo->x;
      l.a.y -= plr->mo->y;
      l.b.y -= plr->mo->y;
      AM_rotate(&l.a.x, &l.a.y, -plr->mo->angle+ANG90);
      AM_rotate(&l.b.x, &l.b.y, -plr->mo->angle+ANG90);
      l.a.x += plr->mo->x;
      l.b.x += plr->mo->x;
      l.a.y += plr->mo->y;
      l.b.y += plr->mo->y;
...
void AM_drawPlayers(void)
{
	...
	if (cheating)
		...
		(cheat_player_arrow, NUMCHEATPLYRLINES, 0,
		 i, WHITE, plr->mo->x, plr->mo->y);
	else
	    AM_drawLineCharacter
		(player_arrow, NUMPLYRLINES, 0, i,
    ...
    for (i=0;i<numsectors;i++)
    {
	  ...
	  while (t)
	  {
        ...
        if (rotatemap)
        {
            ...
            x -= plr->mo->x;
            y -= plr->mo->y;
            AM_rotate(&x, &y, -plr->mo->angle+ANG90);
            x += plr->mo->x;
            y += plr->mo->y;
        }
		///////////////
		...
		(thintriangle_guy, NUMTHINTRIANGLEGUYLINES,
		 16<<FRACBITS, t->angle, colors+lightlev, x, y);
	    t = t->snext;
	  }
    }
}
