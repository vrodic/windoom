...
void D_Display (void)
   {
    ...
    switch (gamestate)
       {
        case GS_LEVEL:
			 ...
			 if (automapactive==2) // vrodic : automap
				AM_Drawer ();
			 ...
			 //vrodic:18-03-98: need to redraw stbar until menu is off, because
    ...
    // draw the view directly // vrodic midified
    ...
    if (gamestate == GS_LEVEL && scaledviewwidth != SCREENWIDTH && viewheight != SCREENHEIGHT && automapactive != 2)
       {
...
void D_PageDrawer (void)
{
  ...
  // vrodic:18-03-98: now fills the screen with a pattern if the screen is
