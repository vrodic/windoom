...
// M_DrawMainMenu
//
// This just draws Title PIC
void M_DrawMainMenu(void)
{
	// vrodic: 19-03-1998 // todo rest off menu stuff
...
void M_DrawOptions(void)
{
	...
	p = W_CacheLumpName(msgNames[showMessages],PU_CACHE);
    V_DrawScaledPatch ((BASEVIDWIDTH-p->width)/2 + OptionsDef.x ,
		OptionsDef.y+LINEHEIGHT*messages,0,p);
...
void M_Drawer (void)
{
    ...
    //vrodic: now that's more readable with a faded background (yeah like Quake...)
    if (menuactive)
	V_DrawFadeScreen ();
